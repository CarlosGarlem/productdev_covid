...
case "$1" in
  webserver)
    airflow initdb
    airflow connections --d --conn_id=fs_data
    airflow connections --d --conn_id=covid_db
    airflow connections --add --conn_id=covid_db --conn_type=mysql --conn_host=db --conn_schema=dm_covid --conn_login=covid --conn_password=covid123 --conn_port=3306 
    airflow connections --add --conn_id=fs_data --conn_type=fs --conn_extra='{"path": "/home/airflow/data/"}'
